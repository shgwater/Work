...
and create_time BETWEEN DATE_ADD(now() ,INTERVAL -8 day) and DATE_ADD(now() ,INTERVAL -1 day)  -- 当前日期到三十天前之间

-- and create_time BETWEEN '2018-01-01 00:00:00' and '2019-01-01 00:00:00'

```
### 蔡轩提供思路
每个仓库的对应的商品的最大和最小的度量分别做出来，然后关联到一起。
``` sql
-- 蔡轩提供
SELECT
	*
...
WHERE
	goods_no = '10001054'
AND logic_warehouse_no = 'L1014'
AND bak_time BETWEEN DATE_ADD(now(), INTERVAL - 8 DAY)
...
AND (
	id = (
		...
		WHERE
			goods_no = A.goods_no
		AND logic_warehouse_no = A.logic_warehouse_no
		AND bak_time BETWEEN DATE_ADD(now(), INTERVAL - 8 DAY)
		AND DATE_ADD(now(), INTERVAL - 1 DAY)
	)
	OR id = (
...
```
### 拼接尝试
按照仓库和商品分组，分别求出最大值对应的行和最小值对应的行，然后关联，取自己需要的行。
``` sql
select a.logic_warehouse_no
,a.goods_no
,a.total_num as start_total_num_7
,b.total_num as start_total_num_7
from (
		select * from stock_day_history
		where id in (
		select min(id)
		FROM
			stock_day_history A
		...
		-- goods_no = '10001054' 
		-- AND logic_warehouse_no = 'L1014'
		 bak_time  BETWEEN DATE_ADD(now() ,INTERVAL -8 day) and DATE_ADD(now() ,INTERVAL -1 day)
...
### 周转完成版
2019年01月04日 12时06分36秒
``` sql
select 
 DATE_ADD(date(now()) ,INTERVAL -1 day) as bak_date
,a.logic_warehouse_no
,a.goods_no
,case when a_90.total_num is null then 0 else 1 end as is_new -- 是否新品 0：否  1：是
,b.total_num as start_total_num_7
